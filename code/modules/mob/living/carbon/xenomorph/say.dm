/mob/living/carbon/xenomorph/say(message)
	...
	if(length(message) >= 2)
		if(can_hivemind_speak && copytext_char(message,1,2) == ";" && length(languages))
		...
		if(length(languages))
			for(var/datum/language/L in languages)
				if(lowertext(channel_prefix) == ":[L.key]" || lowertext(channel_prefix) == ".[L.key]" || uppertext(channel_prefix) == ":[convert_en_key_to_ru_key(L.key)]" || uppertext(channel_prefix) == ".[convert_en_key_to_ru_key(L.key)]")
					verb = L.speech_verb
					speaking = L
					break
